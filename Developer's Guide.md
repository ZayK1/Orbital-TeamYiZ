...
### 🎯 Domain Architecture
### 📊 Plan Creation & Tracking Flow

This diagram illustrates the two distinct paths for creating and tracking Skills versus Habits, from user input to database interaction.

```mermaid
graph TD
    subgraph "User Interaction"
        A["User decides to create a new plan"] --> B{"Choose Plan Type: Skill or Habit"};
    end

    B --> C["SKILL PATH<br/>(Structured Learning)"];
    B --> D["HABIT PATH<br/>(Continuous Tracking)"];

    subgraph "Skill Creation & Tracking"
        C --> E["Input: Skill Name, Difficulty"];
        E --> F["POST /api/v1/plans/skills"];
        F --> G["SkillService calls<br/>AI Service (OpenRouter)"];
        G --> H["AI generates a structured<br/>30-day curriculum"];
        H --> I["SkillRepository saves<br/>the full plan to 'skills' collection"];
        I --> J["Tracking: User marks a day as complete via<br/>PATCH /skills/{id}/days/{day}/complete"];
    end

    subgraph "Habit Creation & Tracking"
        D --> K["Input: Habit Title, Category"];
        K --> L["POST /api/v1/plans/habits"];
        L --> M["HabitService creates a simple<br/>trackable object (No AI)"];
        M --> N["HabitRepository saves<br/>the simple plan to 'habits' collection"];
        N --> O["Tracking: User logs progress via<br/>POST /habits/{id}/checkin"];
        O --> P["HabitService records the check-in<br/>and recalculates all streaks"];
        P --> Q["Data is saved to 'habit_checkins'<br/>and the parent 'habits' document is updated"];
    end
```
```
...
```bash
curl -X POST https://your-api.onrender.com/api/v1/plans/habits/your_habit_id/checkin \
  -H "Authorization: Bearer YOUR_JWT_TOKEN" \
  -H "Content-Type: application/json" \
  -d '{
    "date": "2025-06-23",
...
```javascript
{
  ...
  date: ISODate,               // "YYYY-MM-DD" format stored as ISODate
  completed: Boolean,
...
- **Repositories**: Pure database operations without business logic
...
The following major features have been successfully implemented and are now live in the application:

| Feature | Status | Description |
|---------|--------|-------------|
| 📊 **Plan Management** | ✅ **Complete** | Full CRUD (Create, Read, Update, Delete) functionality for both Skills and Habits. |
| 🎯 **Habit Tracking System** | ✅ **Complete** | Complete system for logging daily habit check-ins with automatic streak calculation. |
| 🚀 **Backend Refactor** | ✅ **Complete** | Solidified the Service and Repository architecture, ensuring clear separation of concerns. |
...
|----------|---------|-------------|----------|
| 🔴 High | **Push Notifications** | Daily learning-reminder notifications via Expo. | July |
| 🟠 Medium | **Habit Analytics** | Visual charts and statistics for habit completion rates and streaks. | July |
| 🟡 Low | **Gamification** | Achievements & learning streak badges for added motivation. | August |
| 🟢 Low | **Social Features** | Ability to share progress or plans with friends. | August |
