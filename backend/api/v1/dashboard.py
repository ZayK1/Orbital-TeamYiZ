...
dashboard_blueprint = Blueprint('dashboard', __name__, url_prefix='/api/v1/dashboard')



@dashboard_blueprint.errorhandler(ValidationError)
def handle_marshmallow_validation(err):
    return jsonify({"error": "Validation failed", "details": err.messages}), 422

@dashboard_blueprint.errorhandler(ValueError)
def handle_value_error(err):
    return jsonify({"error": str(err)}), 400

@dashboard_blueprint.errorhandler(Exception)
